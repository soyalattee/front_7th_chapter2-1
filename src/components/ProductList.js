...
const ProductItem = ({ productId, image, title, lprice }) => `
...
export const ProductList = ({ products, isLoading = false }) => {
  ...
  return `
    <div class="mb-6">
      ...
      `
        ...
        ${!isLoading && products?.length > 0 && products.map(ProductItem).join("")}
        </div>
        ${isLoading ? Loading : ""}
      </div>
    </div>
